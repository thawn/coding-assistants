...
\subtitle{AIfternoon: Agents at Work} % Change Me
\author{\href{mailto:t.korten@hzdr.de}{Till Korten}} % Change Me
\date{March 2, 2026} % Change Me
  ...
  \begin{columns}
    ...
    \begin{column}{0.4\textwidth}
      \begin{itemize}
        ...
        \item Implement whole features, fix bugs, write documentation
        \item Powered by large language models (LLMs) fine-tuned for tool use and code generation
        \item Accelerate prototyping; you stay in control. \textbf{Human-in-the-loop is essential.}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \textcolor{applegreen}{\faCheck} \textbf{Dos:}
      \begin{itemize}
        \item Specify the desired end result, not how to implement it.
        \item Plan the issue (=prompt) well (use a chat model).
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \textcolor{cadmiumred}{\faTimes} \textbf{Don'ts:}
      \begin{itemize}
        \item Micromanage the agent with overly strict prompts.
        \item Just assign a user-submitted issue without refinement.
        \item Use untested generated code you do not understand.
        \item Expect perfect results on the first try.
        \item Live with code just because the agent can't do better
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Agents have good overview of state-of-the-art
    \item Offload routine coding tasks
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item May produce unmaintainable code if not guided properly
    \item Not a replacement for skilled software engineers
    \item Best used as a tool to augment, not replace, human creativity and judgment
  \end{itemize}
\end{frame}
